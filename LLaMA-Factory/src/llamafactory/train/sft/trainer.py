...
from ...model.model_utils.recurft import load_recurft_recurrent_module, save_recurft_recurrent_module
from ..callbacks import SaveProcessorCallback
from ..fp8_utils import configure_fp8_environment, patch_accelerator_for_fp8, verify_fp8_status
from ..trainer_utils import create_custom_optimizer, create_custom_scheduler
from .recurft import compute_recurft_loss
...
def _accumulate_recurft_metrics(
    sums: dict[str, float], counts: dict[str, int], metrics: dict[str, float]
) -> None:
    """Accumulate microbatch metrics for the next Trainer logging window."""
    for key, value in metrics.items():
        sums[key] = sums.get(key, 0.0) + float(value)
        counts[key] = counts.get(key, 0) + 1


def _consume_recurft_metrics(sums: dict[str, float], counts: dict[str, int]) -> dict[str, float]:
    metrics = {key: total / counts[key] for key, total in sums.items() if counts.get(key, 0) > 0}
    sums.clear()
    counts.clear()
    return metrics
...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, *args, **kwargs):
        if self.finetuning_args.use_recurft:
            return_outputs = kwargs.get("return_outputs", False)
            loss, outputs, metrics = compute_recurft_loss(
                model=model,
                inputs=inputs,
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        if self.finetuning_args.use_recurft:
            load_recurft_recurrent_module(model if model is not None else self.model, resume_from_checkpoint)
