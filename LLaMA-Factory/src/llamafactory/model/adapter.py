...
from .model_utils.recurft import (
    attach_recurft_recurrent_module,
    build_recurft_recurrent_module,
    collect_recurft_lora_targets,
)
...
def _restrict_to_existing_trainable_prefix(model: torch.nn.Module, prefix: str) -> tuple[str, ...]:
    """Freeze everything outside a scope without re-enabling parameters frozen by construction."""
    retained = []
    for name, param in model.named_parameters():
        keep_trainable = name.startswith(prefix) and param.requires_grad
...
def _setup_lora_tuning(
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        if adapter_to_resume is not None:  # resume lora training
            if finetuning_args.use_recurft:
                raise ValueError(
                    "Loading an existing RecurFT adapter via `adapter_name_or_path` is not implemented yet. "
                    "Start a new RecurFT adapter or use `resume_from_checkpoint` for an in-progress run."
                )
    ...
    recurft_recurrent_module = None
    recurft_rank_pattern = None
    recurft_alpha_pattern = None
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        if finetuning_args.use_recurft:
            if finetuning_args.finetuning_type != "lora":
                raise ValueError("RecurFT currently only supports LoRA fine-tuning.")

            if model_args.use_kt:
                raise ValueError("RecurFT does not support KTransformers yet.")

            if model_args.use_unsloth:
                raise ValueError("RecurFT does not support Unsloth yet.")

            target_modules, recurft_rank_pattern, recurft_alpha_pattern, recurft_metadata = (
                collect_recurft_lora_targets(model, finetuning_args)
            )
            recurft_recurrent_module = build_recurft_recurrent_module(model, finetuning_args, recurft_metadata)
        elif len(finetuning_args.lora_target) == 1 and finetuning_args.lora_target[0] == "all":
            target_modules = find_all_linear_modules(model, finetuning_args.freeze_vision_tower)
        else:
            target_modules = finetuning_args.lora_target

        if finetuning_args.use_llama_pro and not finetuning_args.use_recurft:
            target_modules = find_expanded_modules(model, target_modules, finetuning_args.freeze_trainable_layers)

        if not finetuning_args.use_recurft:
        ...
        if finetuning_args.finetuning_type == "lora":
            ...
            if finetuning_args.use_recurft:
                lora_rank = finetuning_args.recurft_pre_lora_rank
                lora_alpha = finetuning_args.recurft_pre_lora_alpha or lora_rank * 2
            ...
            if finetuning_args.use_recurft:
                peft_kwargs["rank_pattern"] = recurft_rank_pattern
                peft_kwargs["alpha_pattern"] = recurft_alpha_pattern
        ...
        if model_args.use_kt:
            ...
            model = get_peft_model(model, peft_config)
            if recurft_recurrent_module is not None:
                attach_recurft_recurrent_module(model, recurft_recurrent_module)

    if is_trainable and finetuning_args.use_recurft and finetuning_args.recurft_recurrent_trainable_only:
        trainable_prefix = "recurft_recurrent."
        retained = _restrict_to_existing_trainable_prefix(model, trainable_prefix)

        logger.info_rank0(
            "RecurFT recurrent-only mode freezes the target path and retains {} trainable T/head tensors.".format(
                len(retained)
            )
        )

    if is_trainable and finetuning_args.use_recurft and finetuning_args.recurft_stage1_heads_only:
        trainable_prefixes = (
            "recurft_recurrent.boundary_",
            "recurft_recurrent.token_conditioning_",
        )
        for name, param in model.named_parameters():
            param.requires_grad_(name.startswith(trainable_prefixes))

        logger.info_rank0("RecurFT Stage-1 pilot trains only boundary-head and token-conditioning parameters.")

    if is_trainable and finetuning_args.use_recurft and finetuning_args.recurft_multistep_residual_only:
        trainable_prefix = "recurft_recurrent.multistep_residual_"
        for name, param in model.named_parameters():
            param.requires_grad_(name.startswith(trainable_prefix))

        logger.info_rank0(
            "RecurFT multi-step pilot trains only residual parameters active from rollout step {}.".format(
                finetuning_args.recurft_multistep_residual_start_step
            )
        )

    if is_trainable and finetuning_args.use_recurft and finetuning_args.recurft_multistep_step1_residual_only:
        trainable_prefix = "recurft_recurrent.multistep_step1_residual_"
        for name, param in model.named_parameters():
            param.requires_grad_(name.startswith(trainable_prefix))

        logger.info_rank0("RecurFT step-1 pilot trains only the dedicated first-rollout residual parameters.")
